...
public class HateFriends{
    ...
    public static void main(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "HateFriends");
        job.setJarByClass(HateFriends.class);
        FileInputFormat.addInputPath(job, new Path(args[0]));
        FileOutputFormat.setOutputPath(job, new Path(args[1]));
        
        job.setMapperClass(HateFriendsMapper.class);
        job.setReducerClass(HateFriendsReducer.class);
